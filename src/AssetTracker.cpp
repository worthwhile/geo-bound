...
// overrode this broken function
uint32_t AssetTracker::getGpsTimestamp() {
    struct tm timeinfo = {};
    timeinfo.tm_year = gps.year + 100; // tm_year is years since 1900.  gps.year is 2 digit year.
    timeinfo.tm_mon = gps.month - 1; // gps.month is 1 relative
    timeinfo.tm_mday = gps.day;
    timeinfo.tm_hour = gps.hour; // gps.hour is 1 relative
...
boolean AssetTracker::standBy(){
    return gps.standby();
}

boolean AssetTracker::wakeUp(){
    return gps.wakeup();
}
